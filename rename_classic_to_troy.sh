...
echo "📝 Replacing all case-insensitive instances of 'classic' with 'troy' in file contents..."
...
find "$BASE_DIR" -type f | while read file; do
  ...
  new_filename=$(echo "$filename" | sed -E 's/[Cc][Ll][Aa][Ss][Ss][Ii][Cc]/Troy/g')
...
find "$BASE_DIR" -depth -type d -iname "*classic*" | sort -r | while read dir; do
  new_dir=$(echo "$dir" | sed -E 's/[Cc][Ll][Aa][Ss][Ss][Ii][Cc]/Troy/g')
  if [ "$dir" != "$new_dir" ]; then
    mv "$dir" "$new_dir"
