...
class ClassicProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('classic-product-header')) {
    customElements.define('classic-product-header', ClassicProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('classic-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof ClassicProductHeader) {
      header.connectedCallback();
    }
  });
});

export { ClassicProductHeader };