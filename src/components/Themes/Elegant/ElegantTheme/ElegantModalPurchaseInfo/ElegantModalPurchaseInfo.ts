// ElegantModalPurchaseInfo.ts - Simplified Implementation
...
class ElegantPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('elegant-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('elegant-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="elegant-selection-item">
          <div class="elegant-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="elegant-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="elegant-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="elegant-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="elegant-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="elegant-direct-item">
        <div class="elegant-direct-item-info">
          <div class="elegant-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="elegant-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="elegant-direct-item-pricing">
          <div class="elegant-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="elegant-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="elegant-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="elegant-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'elegant-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, ElegantPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof ElegantPurchaseModal) {
