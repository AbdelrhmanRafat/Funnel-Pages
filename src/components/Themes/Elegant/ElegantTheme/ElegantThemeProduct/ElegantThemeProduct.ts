...
class ElegantProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('elegant-product-header')) {
    customElements.define('elegant-product-header', ElegantProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('elegant-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof ElegantProductHeader) {
      header.connectedCallback();
    }
  });
});

export { ElegantProductHeader };