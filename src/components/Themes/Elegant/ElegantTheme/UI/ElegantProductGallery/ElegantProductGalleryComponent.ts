// ElegantGalleryComponent.ts - Web Component for Gallery
...
class ElegantGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('elegant-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('elegant-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('elegant-gallery')) {
        customElements.define('elegant-gallery', ElegantGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('elegant-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof ElegantGallery) {
            gallery.connectedCallback();
        }
    });
});

export { ElegantGallery };