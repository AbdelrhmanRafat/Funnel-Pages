// ElegantThemeProductFunnel.ts - Web Component for Product Funnel
...
class ElegantProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('elegant-product-funnel')) {
    customElements.define('elegant-product-funnel', ElegantProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('elegant-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof ElegantProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { ElegantProductFunnel };