// TechnoThemeProductFunnel.ts - Web Component for Product Funnel
...
class TechnoProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('techno-product-funnel')) {
    customElements.define('techno-product-funnel', TechnoProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('techno-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof TechnoProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { TechnoProductFunnel };