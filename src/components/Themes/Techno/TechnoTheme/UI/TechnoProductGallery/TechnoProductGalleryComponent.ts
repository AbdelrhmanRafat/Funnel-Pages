// TechnoGalleryComponent.ts - Web Component for Gallery
...
class TechnoGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('techno-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('techno-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('techno-gallery')) {
        customElements.define('techno-gallery', TechnoGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('techno-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof TechnoGallery) {
            gallery.connectedCallback();
        }
    });
});

export { TechnoGallery };