// TechnoModalPurchaseInfo.ts - Simplified Implementation
...
class TechnoPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('techno-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('techno-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="techno-selection-item">
          <div class="techno-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="techno-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="techno-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="techno-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="techno-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="techno-direct-item">
        <div class="techno-direct-item-info">
          <div class="techno-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="techno-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="techno-direct-item-pricing">
          <div class="techno-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="techno-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="techno-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="techno-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'techno-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, TechnoPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof TechnoPurchaseModal) {
