...
class TechnoProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('techno-product-header')) {
    customElements.define('techno-product-header', TechnoProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('techno-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof TechnoProductHeader) {
      header.connectedCallback();
    }
  });
});

export { TechnoProductHeader };