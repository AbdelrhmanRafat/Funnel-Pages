...
interface TechnoDeliveryOptionsSectionProps {
  deliveryOptions: DeliveryOption[];
  currentLang: Language;
}

const TechnoDeliveryOptionsSection: React.FC<TechnoDeliveryOptionsSectionProps> = ({
  deliveryOptions,
  currentLang,
}) => {
  // Delivery store hook
  ...
  return (
    <div className='w-full'>
      <label
        className="techno-form-label-static block mb-2 text-sm font-medium md:text-base md:mb-3"
      ...
      >
        {deliveryOptions.map((option, index) => {
          ...
          return (
            <label key={option.id} className="techno-form-div-payment flex items-start gap-2 md:gap-3" htmlFor={id}>
              <input
                type="radio"
                id={id}
                name="delivery-option"
                value={value}
                className="techno-form-input-radio w-4 h-4 md:w-5 md:h-5"
                defaultChecked={isChecked}
                onChange={() => handleDeliveryChange(id, value)}
              />
              <div className="techno-form-div-content flex-1 ml-3 md:ml-4">
                <div className="flex justify-between items-center">
                  <div className="flex-1 flex flex-col justify-start items-start gap-2">
                    <span className="techno-form-span-payment-label font-medium text-sm md:text-base">
                      {option.label[currentLang]}
                    </span>
                    <span className="techno-form-span-payment-description text-xs md:text-sm">
...
export default TechnoDeliveryOptionsSection;