// BoldNavigationMenuDesktop.ts - Optimized Web Component with Overflow
...
class DesktopNavigation extends HTMLElement {
  ...
  private initializeElements(): void {
    ...
    this.navLinks = this.querySelectorAll('.bold-desktop-nav-link');
  }
    ...
    for (let i = startIndex; i < navItems.length; i++) {
      ...
      if (link) {
        ...
        li.innerHTML = `
          <a 
            href="${href}"
            class="bold-overflow-dropdown-link block px-4 py-2 text-sm rounded transition-colors duration-200 hover:bg-gray-100"
  ...
  private smoothScrollToSection(targetId: string): void {
    ...
    if (targetElement) {
      const header = document.querySelector('.bold-header') as HTMLElement;
