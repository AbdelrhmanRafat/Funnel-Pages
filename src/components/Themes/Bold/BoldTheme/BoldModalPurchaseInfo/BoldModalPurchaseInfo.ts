// BoldModalPurchaseInfo.ts - Simplified Implementation
...
class BoldPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('bold-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('bold-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="bold-selection-item">
          <div class="bold-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="bold-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="bold-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="bold-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="bold-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="bold-direct-item">
        <div class="bold-direct-item-info">
          <div class="bold-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="bold-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="bold-direct-item-pricing">
          <div class="bold-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="bold-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="bold-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="bold-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'bold-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, BoldPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof BoldPurchaseModal) {
