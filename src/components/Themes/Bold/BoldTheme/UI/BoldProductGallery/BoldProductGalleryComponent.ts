// BoldGalleryComponent.ts - Web Component for Gallery
...
class BoldGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('bold-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('bold-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('bold-gallery')) {
        customElements.define('bold-gallery', BoldGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('bold-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof BoldGallery) {
