// BoldThemeProductFunnel.ts - Web Component for Product Funnel
...
class BoldProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('bold-product-funnel')) {
    customElements.define('bold-product-funnel', BoldProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('bold-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof BoldProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { BoldProductFunnel };