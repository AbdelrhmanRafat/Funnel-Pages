...
class BoldProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('bold-product-header')) {
    customElements.define('bold-product-header', BoldProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('bold-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof BoldProductHeader) {
      header.connectedCallback();
    }
  });
});

export { BoldProductHeader };