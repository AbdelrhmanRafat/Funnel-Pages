...
import FreshBundleOptionsContainerReact from "../FreshBundleOptionsContainer/FreshBundleOptionsContainerReact.tsx";
import { useCustomOptionBundleStore } from '../../../../../../lib/stores/customOptionBundleStore.ts';

interface FreshBundleOptionsReactProps {
  data: PurchaseOption[];
  product: Product;
  name: string;
...
const FreshBundleOptionsReact: React.FC<FreshBundleOptionsReactProps> = ({
  data,
  product,
  name,
  currentLang,
  ...
  return (
    <div id="fresh-bundle-options">
      <h3 className="fresh-bundle-options-title text-2xl py-3">
      ...
      <div className="space-y-3">
        {data.map((item, index) => (
          <div key={index}>
            <div className="fresh-bundle-options-group border border-primary rounded-2xl p-1.5 sm:p-2">
              <label
                className="fresh-bundle-options-label group relative overflow-hidden cursor-pointer block rounded-lg"
              ...
              >
                ...
                />
                <div className="fresh-bundle-options-content p-1.5 sm:p-2">
                  <div className="fresh-bundle-options-header grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
                    <div className="sm:col-span-2 space-y-1">
                      <h3 className="fresh-bundle-options-option-title font-bold text-xl leading-tight">
                        {item.title}
                      </h3>
                      <div className="fresh-bundle-options-option-details flex flex-wrap items-center gap-1 sm:gap-2 text-xs">
                        <div className="fresh-bundle-options-unit-badge py-0.5 px-2 rounded-full inline-block">
                          <span>{item.items === 1 ? "" : item.items}</span>
                          <span> {getTranslation("quantityOptions.itemUnit.singular", currentLang)}</span>
                        </div>
                        <span className="hidden sm:inline text-gray-400">•</span>
                        <div className="fresh-bundle-options-price-info flex items-center gap-1 text-xs">
                    ...
                    <div className="flex flex-col items-start sm:items-end justify-center">
                      <div
                        className={`fresh-bundle-options-final-price text-lg sm:text-xl font-bold leading-tight ${
                          item.discount > 0 ? "fresh-bundle-options-price-discount" : "fresh-bundle-options-price-regular"
                        }`}
                      >
                      ...
                      {item.discount > 0 && (
                        <div className="flex flex-col items-start sm:items-end mt-1">
                          <div className="fresh-bundle-options-original-price text-xs line-through text-gray-500">
                            {item.original_total.toLocaleString()}{" "}
                            {getTranslation("productFunnel.currency", currentLang)}
                          </div>
                          <div className="fresh-bundle-options-discount-badge inline-flex items-center text-xs font-medium py-0.5 px-1.5 rounded-full mt-0.5">
                            <svg
                              className="fresh-bundle-options-discount-icon w-2.5 h-2.5"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
              ...
              {isHaveVariant && (
                ...
                >
                  {Array.from({ length: item.items || 0 }).map((_, panelIndex) => (
                    <FreshBundleOptionsContainerReact
                      key={panelIndex}
...
export default FreshBundleOptionsReact;