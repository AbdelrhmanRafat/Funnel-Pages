// FreshThemeProductFunnel.ts - Web Component for Product Funnel
...
class FreshProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('fresh-product-funnel')) {
    customElements.define('fresh-product-funnel', FreshProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('fresh-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof FreshProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { FreshProductFunnel };