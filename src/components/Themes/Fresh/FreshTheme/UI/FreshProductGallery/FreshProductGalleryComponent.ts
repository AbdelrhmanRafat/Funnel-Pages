// FreshGalleryComponent.ts - Web Component for Gallery
...
class FreshGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('fresh-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('fresh-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('fresh-gallery')) {
        customElements.define('fresh-gallery', FreshGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('fresh-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof FreshGallery) {
