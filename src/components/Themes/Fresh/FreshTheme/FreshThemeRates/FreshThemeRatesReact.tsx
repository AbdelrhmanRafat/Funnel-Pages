...
import './FreshThemeRates.css';

interface FreshThemeRatesReactProps {
...
const FreshThemeRatesReact: React.FC<FreshThemeRatesReactProps> = ({
  ratingData,
  currentLang,
}) => {
  ...
  const StarIcon: React.FC<{ star: Star }> = ({ star }) => {
    ...
    switch (star.type) {
      case 'full':
        return (
          <svg className="fresh-rating-star-full" {...baseProps}>
      ...
      case 'half':
        return (
          <svg className="fresh-rating-star-half" {...baseProps}>
            <defs>
              <linearGradient id="halfGradient">
                <stop offset="50%" stopColor="currentColor" />
                <stop offset="50%" stopColor="var(--fresh-color-outline)" />
              </linearGradient>
            </defs>
      ...
      case 'empty':
        return (
          <svg className="fresh-rating-star-empty" {...baseProps}>
            <path d={starPath} fill="currentColor" />
          </svg>
        );
  ...
  return (
    <div id={FunnelFreshComponents.FreshRates} className="fresh-rates">
      <div
        className="fresh-rating-display"
      ...
      >
        ...
        <span className="fresh-rating-number">({rating})</span>
      </div>
    </div>
  );
};

export default FreshThemeRatesReact;