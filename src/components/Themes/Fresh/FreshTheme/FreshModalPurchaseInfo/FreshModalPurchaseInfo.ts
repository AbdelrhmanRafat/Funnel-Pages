// FreshModalPurchaseInfo.ts - Simplified Implementation
...
class FreshPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('fresh-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('fresh-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="fresh-selection-item">
          <div class="fresh-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="fresh-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="fresh-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="fresh-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="fresh-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="fresh-direct-item">
        <div class="fresh-direct-item-info">
          <div class="fresh-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="fresh-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="fresh-direct-item-pricing">
          <div class="fresh-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="fresh-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="fresh-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="fresh-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'fresh-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, FreshPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof FreshPurchaseModal) {
