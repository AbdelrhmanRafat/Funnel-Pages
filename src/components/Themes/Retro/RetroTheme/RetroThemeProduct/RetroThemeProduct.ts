...
class RetroProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('retro-product-header')) {
    customElements.define('retro-product-header', RetroProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('retro-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof RetroProductHeader) {
      header.connectedCallback();
    }
  });
});

export { RetroProductHeader };