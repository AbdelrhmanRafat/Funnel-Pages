// RetroThemeProductFunnel.ts - Web Component for Product Funnel
...
class RetroProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('retro-product-funnel')) {
    customElements.define('retro-product-funnel', RetroProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('retro-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof RetroProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { RetroProductFunnel };