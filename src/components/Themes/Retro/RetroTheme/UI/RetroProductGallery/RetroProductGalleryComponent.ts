// RetroGalleryComponent.ts - Web Component for Gallery
...
class RetroGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('retro-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('retro-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('retro-gallery')) {
        customElements.define('retro-gallery', RetroGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('retro-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof RetroGallery) {
