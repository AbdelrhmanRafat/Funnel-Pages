// RetroModalPurchaseInfo.ts - Simplified Implementation
...
class RetroPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('retro-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('retro-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="retro-selection-item">
          <div class="retro-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="retro-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="retro-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="retro-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="retro-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="retro-direct-item">
        <div class="retro-direct-item-info">
          <div class="retro-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="retro-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="retro-direct-item-pricing">
          <div class="retro-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="retro-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="retro-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="retro-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'retro-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, RetroPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof RetroPurchaseModal) {
