...
class UrbanProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('urban-product-header')) {
    customElements.define('urban-product-header', UrbanProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('urban-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof UrbanProductHeader) {
      header.connectedCallback();
    }
  });
});

export { UrbanProductHeader };