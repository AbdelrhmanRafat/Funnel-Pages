...
interface UrbanNotesSectionProps {
  currentLang: Language;
}

const UrbanNotesSection: React.FC<UrbanNotesSectionProps> = ({
  currentLang,
}) => {
  // Form store hooks
  const {
  ...
  return (
    <div className="urban-form-div-notes">
      <div className="urban-form-div-group">
        <textarea
          ...
          placeholder=""
          className={`urban-form-input-base urban-form-textarea-responsive ${isArabic ? "text-right" : "text-left"} min-h-[100px] md:min-h-[120px] resize-y text-sm md:text-base ${
            notesField.touched && notesField.errorMessage ? 'urban-form-input-error' : 
            notesField.touched && notesField.isValid ? 'urban-form-input-success' : ''
          }`}
...
export default UrbanNotesSection;