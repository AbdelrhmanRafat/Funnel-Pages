// UrbanSelectOptionsBundles.ts - Enhanced with Fixed Naming Convention
...
const CSS_CLASSES = {
  // Selection states
  SELECTED_COLOR: 'urban-bundle-options-container-selected-color',
  SELECTED_SIZE: 'urban-bundle-options-container-selected-size',
  SELECTED_GENERIC: 'urban-selected',
  
  // Option types
  COLOR_OPTION: 'urban-bundle-options-container-color-option',
  SIZE_OPTION: 'urban-bundle-options-container-size-option',
  
  // Availability states
  OPTION_AVAILABLE: 'urban-bundle-options-container-option-available',
  OPTION_DISABLED: 'urban-bundle-options-container-option-disabled',
  OPTION_TRANSITIONING: 'urban-bundle-options-container-option-transitioning'
} as const;

class UrbanSelectOptionsBundles extends HTMLElement implements Observer<BundleState>, Observer<CustomOptionBundlesState> {
  ...
  private initializeSettings(): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to initialize settings:', error);
    }
  }
  ...
  private initializeElements(): void {
    ...
    if (!this.firstOptionElements?.length && !this.secondOptionElements?.length) {
      console.warn('UrbanSelectOptionsBundles: No option elements found');
    }
  }
  ...
  public update(subject: Subject<BundleState | CustomOptionBundlesState>): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Observer update failed:', error);
    }
  }
  ...
  private handleFirstOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('UrbanSelectOptionsBundles: First option selection failed:', error);
    }
  }
  ...
  private handleSecondOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Second option selection failed:', error);
    }
  }
  ...
  private applySelectionStyle(element: HTMLElement): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to apply selection style:', error);
    }
  }
  ...
  private filterSecondOptions(firstValue: string): void {
    ...
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to filter second options:', error);
    }
  }
  ...
  private toggleElementAvailability(element: HTMLElement, isAvailable: boolean): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to toggle element availability:', error);
    }
  }
  ...
  private syncUIWithObserver(panelOption: any): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to sync UI with observer:', error);
    }
  }
  ...
  private updateVisualSelections(panelOption: any): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to update visual selections:', error);
    }
  }
  ...
  private findSkuId(): number | null {
    ...
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to find SKU ID:', error);
      return null;
    }
  }
  ...
  private findImageUrl(): string | null {
    ...
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to find image URL:', error);
      return null;
    }
  }
  ...
  public clearSelections(): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to clear selections:', error);
    }
  }
  ...
  public forceRefreshSelections(): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to refresh selections:', error);
    }
  }
  ...
  public validateSelectionStates(): boolean {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to validate selection states:', error);
      return false;
    }
  }
  ...
  public repairSelections(): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to repair selections:', error);
    }
  }
  ...
  public resetToInitialState(): void {
    try {
      ...
      console.error('UrbanSelectOptionsBundles: Failed to reset to initial state:', error);
    }
  }
...
class UrbanSelectOptionsBundlesManager {
  private static readonly COMPONENT_NAME = 'urban-select-options-bundles';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, UrbanSelectOptionsBundles);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedComponents.forEach(component => {
      if (component instanceof UrbanSelectOptionsBundles) {
...
UrbanSelectOptionsBundlesManager.initialize();

export { UrbanSelectOptionsBundles };