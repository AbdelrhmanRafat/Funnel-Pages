// UrbanGalleryComponent.ts - Web Component for Gallery
...
class UrbanGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('urban-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('urban-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('urban-gallery')) {
        customElements.define('urban-gallery', UrbanGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('urban-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof UrbanGallery) {
