// UrbanModalPurchaseInfo.ts - Simplified Implementation
...
class UrbanPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('urban-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('urban-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="urban-selection-item">
          <div class="urban-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="urban-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="urban-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="urban-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="urban-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="urban-direct-item">
        <div class="urban-direct-item-info">
          <div class="urban-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="urban-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="urban-direct-item-pricing">
          <div class="urban-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="urban-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="urban-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="urban-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'urban-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, UrbanPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof UrbanPurchaseModal) {
