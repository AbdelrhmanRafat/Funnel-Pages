// ProGalleryComponent.ts - Web Component for Gallery
...
class ProGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('pro-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('pro-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('pro-gallery')) {
        customElements.define('pro-gallery', ProGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('pro-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof ProGallery) {
