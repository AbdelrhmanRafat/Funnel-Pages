...
import ProBundleOptionsContainerReact from "../ProBundleOptionsContainer/ProBundleOptionsContainerReact.tsx";
import { useCustomOptionBundleStore } from '../../../../../../lib/stores/customOptionBundleStore.ts';

interface ProBundleOptionsReactProps {
  data: PurchaseOption[];
  product: Product;
  name: string;
...
const ProBundleOptionsReact: React.FC<ProBundleOptionsReactProps> = ({
  data,
  product,
  name,
  currentLang,
  ...
  return (
    <div id="pro-bundle-options">
      <h3 className="pro-bundle-options-title text-2xl py-3">
      ...
      <div className="space-y-3">
        {data.map((item, index) => (
          <div key={index}>
            <div className="pro-bundle-options-group border border-primary rounded-2xl p-1.5 sm:p-2">
              <label
                className="pro-bundle-options-label group relative overflow-hidden cursor-pointer block rounded-lg"
              ...
              >
                ...
                />
                <div className="pro-bundle-options-content p-1.5 sm:p-2">
                  <div className="pro-bundle-options-header grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
                    <div className="sm:col-span-2 space-y-1">
                      <h3 className="pro-bundle-options-option-title font-bold text-xl leading-tight">
                        {item.title}
                      </h3>
                      <div className="pro-bundle-options-option-details flex flex-wrap items-center gap-1 sm:gap-2 text-xs">
                        <div className="pro-bundle-options-unit-badge py-0.5 px-2 rounded-full inline-block">
                          <span>{item.items === 1 ? "" : item.items}</span>
                          <span> {getTranslation("quantityOptions.itemUnit.singular", currentLang)}</span>
                        </div>
                        <span className="hidden sm:inline text-gray-400">•</span>
                        <div className="pro-bundle-options-price-info flex items-center gap-1 text-xs">
                    ...
                    <div className="flex flex-col items-start sm:items-end justify-center">
                      <div
                        className={`pro-bundle-options-final-price text-lg sm:text-xl font-bold leading-tight ${
                          item.discount > 0 ? "pro-bundle-options-price-discount" : "pro-bundle-options-price-regular"
                        }`}
                      >
                      ...
                      {item.discount > 0 && (
                        <div className="flex flex-col items-start sm:items-end mt-1">
                          <div className="pro-bundle-options-original-price text-xs line-through text-gray-500">
                            {item.original_total.toLocaleString()}{" "}
                            {getTranslation("productFunnel.currency", currentLang)}
                          </div>
                          <div className="pro-bundle-options-discount-badge inline-flex items-center text-xs font-medium py-0.5 px-1.5 rounded-full mt-0.5">
                            <svg
                              className="pro-bundle-options-discount-icon w-2.5 h-2.5"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
              ...
              {isHaveVariant && (
                ...
                >
                  {Array.from({ length: item.items || 0 }).map((_, panelIndex) => (
                    <ProBundleOptionsContainerReact
                      key={panelIndex}
...
export default ProBundleOptionsReact;