// ProThemeProductFunnel.ts - Web Component for Product Funnel
...
class ProProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('pro-product-funnel')) {
    customElements.define('pro-product-funnel', ProProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('pro-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof ProProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { ProProductFunnel };