// ProModalPurchaseInfo.ts - Simplified Implementation
...
class ProPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('pro-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('pro-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="pro-selection-item">
          <div class="pro-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="pro-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="pro-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="pro-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="pro-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="pro-direct-item">
        <div class="pro-direct-item-info">
          <div class="pro-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="pro-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="pro-direct-item-pricing">
          <div class="pro-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="pro-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="pro-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="pro-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'pro-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, ProPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof ProPurchaseModal) {
