...
class ProProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('pro-product-header')) {
    customElements.define('pro-product-header', ProProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('pro-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof ProProductHeader) {
      header.connectedCallback();
    }
  });
});

export { ProProductHeader };