// TroyNavigationMenuDesktop.ts - Optimized Web Component with Overflow
...
class DesktopNavigation extends HTMLElement {
  ...
  private initializeElements(): void {
    ...
    this.navLinks = this.querySelectorAll('.troy-desktop-nav-link');
  }
    ...
    for (let i = startIndex; i < navItems.length; i++) {
      ...
      if (link) {
        ...
        li.innerHTML = `
          <a 
            href="${href}"
            class="troy-overflow-dropdown-link block px-4 py-2 text-sm rounded transition-colors duration-200 hover:bg-gray-100"
