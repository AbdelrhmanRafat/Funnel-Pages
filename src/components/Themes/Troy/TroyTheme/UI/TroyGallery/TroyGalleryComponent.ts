// troyGalleryComponent.ts - Web Component for Gallery
...
class troyGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('troy-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('troy-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('troy-gallery')) {
        customElements.define('troy-gallery', troyGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('troy-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof troyGallery) {
