...
class TroyProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('troy-product-header')) {
    customElements.define('troy-product-header', TroyProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('troy-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof TroyProductHeader) {
      header.connectedCallback();
    }
  });
});

export { TroyProductHeader };