// TroyModalPurchaseInfo.ts - Simplified Implementation
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="troy-selection-item">
          <div class="troy-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="troy-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="troy-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="troy-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="troy-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="troy-direct-item">
        <div class="troy-direct-item-info">
          <div class="troy-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="troy-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="troy-direct-item-pricing">
          <div class="troy-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="troy-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="troy-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="troy-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'troy-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, TroyPurchaseModal);
    }
  }
