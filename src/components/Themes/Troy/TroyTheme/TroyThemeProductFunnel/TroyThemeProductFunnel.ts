// troyThemeProductFunnel.ts - Web Component for Product Funnel
...
class troyProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('troy-product-funnel')) {
    customElements.define('troy-product-funnel', troyProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('troy-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof troyProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { troyProductFunnel };