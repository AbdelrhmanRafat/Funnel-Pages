// ZenModalPurchaseInfo.ts - Simplified Implementation
...
class ZenPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('zen-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('zen-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="zen-selection-item">
          <div class="zen-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="zen-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="zen-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="zen-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="zen-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="zen-direct-item">
        <div class="zen-direct-item-info">
          <div class="zen-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="zen-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="zen-direct-item-pricing">
          <div class="zen-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="zen-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="zen-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="zen-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'zen-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, ZenPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof ZenPurchaseModal) {
