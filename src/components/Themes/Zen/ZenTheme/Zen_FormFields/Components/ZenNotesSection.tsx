...
interface ZenNotesSectionProps {
  currentLang: Language;
}

const ZenNotesSection: React.FC<ZenNotesSectionProps> = ({
  currentLang,
}) => {
  // Form store hooks
  const {
  ...
  return (
    <div className="zen-form-div-notes">
      <div className="zen-form-div-group">
        <textarea
          ...
          placeholder=""
          className={`zen-form-input-base zen-form-textarea-responsive ${isArabic ? "text-right" : "text-left"} min-h-[100px] md:min-h-[120px] resize-y text-sm md:text-base ${
            notesField.touched && notesField.errorMessage ? 'zen-form-input-error' : 
            notesField.touched && notesField.isValid ? 'zen-form-input-success' : ''
          }`}
...
export default ZenNotesSection;