...
class ZenProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('zen-product-header')) {
    customElements.define('zen-product-header', ZenProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('zen-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof ZenProductHeader) {
      header.connectedCallback();
    }
  });
});

export { ZenProductHeader };