// ZenVisitorsFlashAnimation.ts

interface ZenVisitor {
    image: string;
    label: string;
  }
  
  interface ZenVisitorsConfig {
    displayTime: number;
    flashDuration: number;
  }
  
  class ZenVisitorsFlashAnimation extends HTMLElement {
    private visitorsData: ZenVisitor[] = [];
    private config: ZenVisitorsConfig;
    ...
    private initializeElements(): void {
      this.imageElement = this.querySelector('#zen-visitor-img') as HTMLImageElement;
      this.descriptionElement = this.querySelector('#zen-visitor-desc') as HTMLElement;
      
      if (!this.imageElement || !this.descriptionElement) {
        console.error('ZenVisitorsFlashAnimation: Required elements not found');
      }
    }
  
    private loadConfiguration(): void {
      const displayTime = this.dataset.zenDisplayTime;
      const flashDuration = this.dataset.zenFlashDuration;
    ...
    private loadVisitorsData(): void {
      const visitorsDataAttribute = this.dataset.zenVisitors;
      
      if (visitorsDataAttribute) {
        try {
          this.visitorsData = JSON.parse(visitorsDataAttribute);
        } catch (error) {
          console.error('ZenVisitorsFlashAnimation: Failed to parse visitors data:', error);
          this.visitorsData = [];
        }
      }
    }
    ...
    private hideVisitor(callback: () => void): void {
      const imageContainer = this.querySelector('.zen-visitor-image') as HTMLElement;
      const description = this.querySelector('.zen-visitor-description') as HTMLElement;
      
      if (imageContainer && description) {
        imageContainer.classList.add('zen-visitor-hidden');
        description.classList.add('zen-visitor-hidden');
      }
    ...
    private flashVisitor(): void {
      ...
      imageContainer.classList.remove('zen-visitor-hidden');
      description.classList.remove('zen-visitor-hidden');
      
      imageContainer.classList.add('zen-visitor-flash', 'zen-visitor-visible');
      description.classList.add('zen-visitor-flash', 'zen-visitor-visible');
      
      // Remove flash class after animation completes
      setTimeout(() => {
        imageContainer.classList.remove('zen-visitor-flash');
        description.classList.remove('zen-visitor-flash');
    ...
    public updateVisitors(newVisitors: ZenVisitor[]): void {
    ...
    public updateConfiguration(newConfig: Partial<ZenVisitorsConfig>): void {
    ...
    public getVisitorsData(): ZenVisitor[] {
  ...
  customElements.define('zen-visitors-animation', ZenVisitorsFlashAnimation);
  
  // Export for potential external use
  export { ZenVisitorsFlashAnimation, type ZenVisitor, type ZenVisitorsConfig };