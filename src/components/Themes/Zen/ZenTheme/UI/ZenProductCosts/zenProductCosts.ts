// ZenThemeProductFunnel.ts - Web Component for Product Funnel
...
class ZenProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('zen-product-funnel')) {
    customElements.define('zen-product-funnel', ZenProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('zen-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof ZenProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { ZenProductFunnel };