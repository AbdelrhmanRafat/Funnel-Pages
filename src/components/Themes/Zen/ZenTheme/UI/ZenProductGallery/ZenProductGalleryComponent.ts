// ZenGalleryComponent.ts - Web Component for Gallery
...
class ZenGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('zen-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('zen-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('zen-gallery')) {
        customElements.define('zen-gallery', ZenGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('zen-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof ZenGallery) {
