// NeonModalPurchaseInfo.ts - Simplified Implementation
...
class NeonPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('neon-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('neon-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="neon-selection-item">
          <div class="neon-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="neon-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="neon-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="neon-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="neon-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="neon-direct-item">
        <div class="neon-direct-item-info">
          <div class="neon-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="neon-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="neon-direct-item-pricing">
          <div class="neon-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="neon-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="neon-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="neon-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'neon-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, NeonPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof NeonPurchaseModal) {
