...
class NeonProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('neon-product-header')) {
    customElements.define('neon-product-header', NeonProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('neon-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof NeonProductHeader) {
      header.connectedCallback();
    }
  });
});

export { NeonProductHeader };