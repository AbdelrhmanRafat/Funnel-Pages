// NeonThemeProductFunnel.ts - Web Component for Product Funnel
...
class NeonProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('neon-product-funnel')) {
    customElements.define('neon-product-funnel', NeonProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('neon-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof NeonProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { NeonProductFunnel };