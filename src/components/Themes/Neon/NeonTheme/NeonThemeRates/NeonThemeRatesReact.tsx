...
import './NeonThemeRates.css';

interface NeonThemeRatesReactProps {
...
const NeonThemeRatesReact: React.FC<NeonThemeRatesReactProps> = ({
  ratingData,
  currentLang,
}) => {
  ...
  const StarIcon: React.FC<{ star: Star }> = ({ star }) => {
    ...
    switch (star.type) {
      case 'full':
        return (
          <svg className="neon-rating-star-full" {...baseProps}>
      ...
      case 'half':
        return (
          <svg className="neon-rating-star-half" {...baseProps}>
            <defs>
              <linearGradient id="halfGradient">
                <stop offset="50%" stopColor="currentColor" />
                <stop offset="50%" stopColor="var(--neon-color-outline)" />
              </linearGradient>
            </defs>
      ...
      case 'empty':
        return (
          <svg className="neon-rating-star-empty" {...baseProps}>
            <path d={starPath} fill="currentColor" />
          </svg>
        );
  ...
  return (
    <div id={FunnelNeonComponents.NeonRates} className="neon-rates">
      <div
        className="neon-rating-display"
      ...
      >
        ...
        <span className="neon-rating-number">({rating})</span>
      </div>
    </div>
  );
};

export default NeonThemeRatesReact;