...
interface NeonNotesSectionProps {
  currentLang: Language;
}

const NeonNotesSection: React.FC<NeonNotesSectionProps> = ({
  currentLang,
}) => {
  // Form store hooks
  const {
  ...
  return (
    <div className="neon-form-div-notes">
      <div className="neon-form-div-group">
        <textarea
          ...
          placeholder=""
          className={`neon-form-input-base neon-form-textarea-responsive ${isArabic ? "text-right" : "text-left"} min-h-[100px] md:min-h-[120px] resize-y text-sm md:text-base ${
            notesField.touched && notesField.errorMessage ? 'neon-form-input-error' : 
            notesField.touched && notesField.isValid ? 'neon-form-input-success' : ''
          }`}
...
export default NeonNotesSection;