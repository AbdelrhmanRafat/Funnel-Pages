...
class NasaProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('nasa-product-header')) {
    customElements.define('nasa-product-header', NasaProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('nasa-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof NasaProductHeader) {
      header.connectedCallback();
    }
  });
});

export { NasaProductHeader };