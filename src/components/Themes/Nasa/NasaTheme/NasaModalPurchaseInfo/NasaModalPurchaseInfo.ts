// NasaModalPurchaseInfo.ts - Simplified Implementation
...
class NasaPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('nasa-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('nasa-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="nasa-selection-item">
          <div class="nasa-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="nasa-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="nasa-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="nasa-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="nasa-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="nasa-direct-item">
        <div class="nasa-direct-item-info">
          <div class="nasa-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="nasa-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="nasa-direct-item-pricing">
          <div class="nasa-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="nasa-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="nasa-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="nasa-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'nasa-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, NasaPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof NasaPurchaseModal) {
