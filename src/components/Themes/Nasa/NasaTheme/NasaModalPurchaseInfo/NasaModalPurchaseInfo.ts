...
class nasaPurchaseModal extends HTMLElement {
  ...
  private createFloatingEmojis(): void {
    const emojis = ['🎉', '✨', '🎊', '🌟', '💫'];
    const container = this.querySelector('.nasa-modal-container');
    if (!container) return;
  ...
  private populateQuantityInfo(quantityData: SelectedOffer | null): void {
    ...
    if (quantityData.discount > 0) {
      this.querySelector('[data-modal-discount-info]')?.classList.add('nasa-discount-info');
    }
  }
    ...
    container.innerHTML = colorSizeOptions.map(option => `
      <div class="nasa-selection-item">
        ...
        <div class="nasa-selection-details flex justify-center gap-3 items-center">
          <div class="nasa-color-display"><span>${option.color}</span></div>
          <div class="nasa-size-display"><span>${option.size}</span></div>
        </div>
      </div>
    `).join('');
  }
...
const initializeModal = () => {
  if (!customElements.get('nasa-purchase-modal')) {
    customElements.define('nasa-purchase-modal', nasaPurchaseModal);
  }
};
document.addEventListener('DOMContentLoaded', initializeModal);
document.addEventListener('astro:page-load', () => {
  document.querySelectorAll('nasa-purchase-modal:not(:defined)')
    .forEach(modal => {
