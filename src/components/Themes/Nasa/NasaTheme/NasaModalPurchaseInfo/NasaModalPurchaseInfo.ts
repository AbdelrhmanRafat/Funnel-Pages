...
class nasaModalPurchaseInfo {
  ...
  constructor() {
    this.modal = document.getElementById('nasa-purchase-modal');
    this.overlay = document.getElementById('nasa-modal-overlay');
    this.closeButton = document.getElementById('nasa-modal-close');
    this.cancelButton = document.getElementById('nasa-modal-cancel');
    ...
    this.celebrationCloseButton = document.getElementById('nasa-modal-close-celebration');
  ...
  private openModal(): void {
    if (this.modal) {
      this.modal.classList.add('nasa-modal-open');
  ...
  private closeModal(): void {
    if (this.modal) {
      this.modal.classList.remove('nasa-modal-open');
  ...
  private createFloatingEmojis(): void {
    const emojis = ['🎉', '✨', '🎊', '🌟', '💫'];
    const modalContainer = document.querySelector('.nasa-modal-container');
  ...
  private populateQuantityInfo(quantityData: QuantityItem | null): void {
    ...
    if (quantityData.discount > 0) {
      ...
      if (discountElement) {
        discountElement.classList.add('nasa-discount-info');
      }
    } else {
  ...
  private populateColorSizeInfo(colorSizeOptions: ColorSizeOption[]): void {
    ...
    colorSizeOptions.forEach((option) => {
      ...
      selectionItem.className = 'nasa-selection-item';
      
      selectionItem.innerHTML = `
        <div class="nasa-panel-info">القطعة ${option.panelIndex}</div>
        <div class="nasa-selection-details flex justify-center gap-3 items-center">
          <div class="nasa-color-display">
            <span>${option.color}</span>
          </div>
          <div class="nasa-size-display">
