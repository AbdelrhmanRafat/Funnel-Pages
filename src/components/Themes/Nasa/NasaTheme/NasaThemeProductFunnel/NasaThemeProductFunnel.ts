// nasaThemeProductFunnel.ts - Web Component for Product Funnel
...
class nasaProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('nasa-product-funnel')) {
    customElements.define('nasa-product-funnel', nasaProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('nasa-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof nasaProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { nasaProductFunnel };