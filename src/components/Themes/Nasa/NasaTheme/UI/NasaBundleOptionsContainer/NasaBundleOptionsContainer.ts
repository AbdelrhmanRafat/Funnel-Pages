// NasaSelectOptionsBundles.ts - Enhanced with Fixed Naming Convention
...
const CSS_CLASSES = {
  // Selection states
  SELECTED_COLOR: 'nasa-bundle-options-container-selected-color',
  SELECTED_SIZE: 'nasa-bundle-options-container-selected-size',
  SELECTED_GENERIC: 'nasa-selected',
  
  // Option types
  COLOR_OPTION: 'nasa-bundle-options-container-color-option',
  SIZE_OPTION: 'nasa-bundle-options-container-size-option',
  
  // Availability states
  OPTION_AVAILABLE: 'nasa-bundle-options-container-option-available',
  OPTION_DISABLED: 'nasa-bundle-options-container-option-disabled',
  OPTION_TRANSITIONING: 'nasa-bundle-options-container-option-transitioning'
} as const;

class NasaSelectOptionsBundles extends HTMLElement implements Observer<BundleState>, Observer<CustomOptionBundlesState> {
  ...
  private initializeSettings(): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to initialize settings:', error);
    }
  }
  ...
  private initializeElements(): void {
    ...
    if (!this.firstOptionElements?.length && !this.secondOptionElements?.length) {
      console.warn('NasaSelectOptionsBundles: No option elements found');
    }
  }
  ...
  public update(subject: Subject<BundleState | CustomOptionBundlesState>): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Observer update failed:', error);
    }
  }
  ...
  private handleFirstOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('NasaSelectOptionsBundles: First option selection failed:', error);
    }
  }
  ...
  private handleSecondOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('NasaSelectOptionsBundles: Second option selection failed:', error);
    }
  }
  ...
  private applySelectionStyle(element: HTMLElement): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to apply selection style:', error);
    }
  }
  ...
  private filterSecondOptions(firstValue: string): void {
    ...
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to filter second options:', error);
    }
  }
  ...
  private toggleElementAvailability(element: HTMLElement, isAvailable: boolean): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to toggle element availability:', error);
    }
  }
  ...
  private syncUIWithObserver(panelOption: any): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to sync UI with observer:', error);
    }
  }
  ...
  private updateVisualSelections(panelOption: any): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to update visual selections:', error);
    }
  }
  ...
  private findSkuId(): number | null {
    ...
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to find SKU ID:', error);
      return null;
    }
  }
  ...
  private findImageUrl(): string | null {
    ...
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to find image URL:', error);
      return null;
    }
  }
  ...
  public clearSelections(): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to clear selections:', error);
    }
  }
  ...
  public forceRefreshSelections(): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to refresh selections:', error);
    }
  }
  ...
  public validateSelectionStates(): boolean {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to validate selection states:', error);
      return false;
    }
  }
  ...
  public repairSelections(): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to repair selections:', error);
    }
  }
  ...
  public resetToInitialState(): void {
    try {
      ...
      console.error('NasaSelectOptionsBundles: Failed to reset to initial state:', error);
    }
  }
...
class NasaSelectOptionsBundlesManager {
  private static readonly COMPONENT_NAME = 'nasa-select-options-bundles';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, NasaSelectOptionsBundles);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedComponents.forEach(component => {
      if (component instanceof NasaSelectOptionsBundles) {
...
NasaSelectOptionsBundlesManager.initialize();

export { NasaSelectOptionsBundles };