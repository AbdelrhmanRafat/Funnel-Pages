// nasaGalleryComponent.ts - Web Component for Gallery
...
class nasaGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('nasa-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('nasa-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('nasa-gallery')) {
        customElements.define('nasa-gallery', nasaGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('nasa-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof nasaGallery) {
