// nasaDynamicPannelContainer.ts - Web Component for Select Options with Observer Integration
...
class nasaSelectOptions extends HTMLElement implements Observer<QuantityState>, Observer<ColorSizeState> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('nasa-select-options')) {
    customElements.define('nasa-select-options', nasaSelectOptions);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const selectOptions = document.querySelectorAll('nasa-select-options:not(:defined)');
  selectOptions.forEach(option => {
    if (option instanceof nasaSelectOptions) {
