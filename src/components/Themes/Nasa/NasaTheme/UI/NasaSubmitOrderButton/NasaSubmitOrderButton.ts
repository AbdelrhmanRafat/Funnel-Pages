// nasaSubmitOrderButton.ts - Web Component for Submit Order with Validation
import {
  isValidFullName,
  isValidPhoneNumber,
  isValidEmail,
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('nasa-submit-order')) {
    customElements.define('nasa-submit-order', nasaSubmitOrder);
  }
});
// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const submitOrders = document.querySelectorAll('nasa-submit-order:not(:defined)');
  submitOrders.forEach(submitOrder => {
    if (submitOrder instanceof nasaSubmitOrder) {
