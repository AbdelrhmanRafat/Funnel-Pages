// ArabicTouchModalPurchaseInfo.ts - Simplified Implementation
...
class ArabicTouchPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('arabictouch-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('arabictouch-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="arabictouch-selection-item">
          <div class="arabictouch-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="arabictouch-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="arabictouch-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="arabictouch-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="arabictouch-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="arabictouch-direct-item">
        <div class="arabictouch-direct-item-info">
          <div class="arabictouch-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="arabictouch-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="arabictouch-direct-item-pricing">
          <div class="arabictouch-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="arabictouch-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="arabictouch-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="arabictouch-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'arabictouch-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, ArabicTouchPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof ArabicTouchPurchaseModal) {
...
export { ArabicTouchPurchaseModal };