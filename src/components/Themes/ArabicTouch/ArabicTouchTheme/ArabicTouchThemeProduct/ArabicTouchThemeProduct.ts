...
class ArabicTouchProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('arabictouch-product-header')) {
    customElements.define('arabictouch-product-header', ArabicTouchProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('arabictouch-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof ArabicTouchProductHeader) {
      header.connectedCallback();
    }
  });
});

export { ArabicTouchProductHeader };