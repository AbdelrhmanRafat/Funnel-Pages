// ArabicTouchThemeProductFunnel.ts - Web Component for Product Funnel
...
class ArabicTouchProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('arabictouch-product-funnel')) {
    customElements.define('arabictouch-product-funnel', ArabicTouchProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('arabictouch-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof ArabicTouchProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { ArabicTouchProductFunnel };