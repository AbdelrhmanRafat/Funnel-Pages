// ArabicTouchGalleryComponent.ts - Web Component for Gallery
...
class ArabicTouchGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('arabictouch-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('arabictouch-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('arabictouch-gallery')) {
        customElements.define('arabictouch-gallery', ArabicTouchGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('arabictouch-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof ArabicTouchGallery) {
            gallery.connectedCallback();
        }
    });
});

export { ArabicTouchGallery };