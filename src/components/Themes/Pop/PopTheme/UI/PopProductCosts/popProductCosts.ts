// PopThemeProductFunnel.ts - Web Component for Product Funnel
...
class PopProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('pop-product-funnel')) {
    customElements.define('pop-product-funnel', PopProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('pop-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof PopProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { PopProductFunnel };