// PopSelectOptionsBundles.ts - Enhanced with Fixed Naming Convention
...
const CSS_CLASSES = {
  // Selection states
  SELECTED_COLOR: 'pop-bundle-options-container-selected-color',
  SELECTED_SIZE: 'pop-bundle-options-container-selected-size',
  SELECTED_GENERIC: 'pop-selected',
  
  // Option types
  COLOR_OPTION: 'pop-bundle-options-container-color-option',
  SIZE_OPTION: 'pop-bundle-options-container-size-option',
  
  // Availability states
  OPTION_AVAILABLE: 'pop-bundle-options-container-option-available',
  OPTION_DISABLED: 'pop-bundle-options-container-option-disabled',
  OPTION_TRANSITIONING: 'pop-bundle-options-container-option-transitioning'
} as const;

class PopSelectOptionsBundles extends HTMLElement implements Observer<BundleState>, Observer<CustomOptionBundlesState> {
  ...
  private initializeSettings(): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to initialize settings:', error);
    }
  }
  ...
  private initializeElements(): void {
    ...
    if (!this.firstOptionElements?.length && !this.secondOptionElements?.length) {
      console.warn('PopSelectOptionsBundles: No option elements found');
    }
  }
  ...
  public update(subject: Subject<BundleState | CustomOptionBundlesState>): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Observer update failed:', error);
    }
  }
  ...
  private handleFirstOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('PopSelectOptionsBundles: First option selection failed:', error);
    }
  }
  ...
  private handleSecondOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('PopSelectOptionsBundles: Second option selection failed:', error);
    }
  }
  ...
  private applySelectionStyle(element: HTMLElement): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to apply selection style:', error);
    }
  }
  ...
  private filterSecondOptions(firstValue: string): void {
    ...
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to filter second options:', error);
    }
  }
  ...
  private toggleElementAvailability(element: HTMLElement, isAvailable: boolean): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to toggle element availability:', error);
    }
  }
  ...
  private syncUIWithObserver(panelOption: any): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to sync UI with observer:', error);
    }
  }
  ...
  private updateVisualSelections(panelOption: any): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to update visual selections:', error);
    }
  }
  ...
  private findSkuId(): number | null {
    ...
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to find SKU ID:', error);
      return null;
    }
  }
  ...
  private findImageUrl(): string | null {
    ...
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to find image URL:', error);
      return null;
    }
  }
  ...
  public clearSelections(): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to clear selections:', error);
    }
  }
  ...
  public forceRefreshSelections(): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to refresh selections:', error);
    }
  }
  ...
  public validateSelectionStates(): boolean {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to validate selection states:', error);
      return false;
    }
  }
  ...
  public repairSelections(): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to repair selections:', error);
    }
  }
  ...
  public resetToInitialState(): void {
    try {
      ...
      console.error('PopSelectOptionsBundles: Failed to reset to initial state:', error);
    }
  }
...
class PopSelectOptionsBundlesManager {
  private static readonly COMPONENT_NAME = 'pop-select-options-bundles';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, PopSelectOptionsBundles);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedComponents.forEach(component => {
      if (component instanceof PopSelectOptionsBundles) {
...
PopSelectOptionsBundlesManager.initialize();

export { PopSelectOptionsBundles };