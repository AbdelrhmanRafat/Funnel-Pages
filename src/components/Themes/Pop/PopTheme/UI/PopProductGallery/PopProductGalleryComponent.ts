// PopGalleryComponent.ts - Web Component for Gallery
...
class PopGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('pop-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('pop-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('pop-gallery')) {
        customElements.define('pop-gallery', PopGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('pop-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof PopGallery) {
