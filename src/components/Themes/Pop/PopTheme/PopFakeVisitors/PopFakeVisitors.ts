// PopVisitorsFlashAnimation.ts

interface PopVisitor {
    image: string;
    label: string;
  }
  
  interface PopVisitorsConfig {
    displayTime: number;
    flashDuration: number;
  }
  
  class PopVisitorsFlashAnimation extends HTMLElement {
    private visitorsData: PopVisitor[] = [];
    private config: PopVisitorsConfig;
    ...
    private initializeElements(): void {
      this.imageElement = this.querySelector('#pop-visitor-img') as HTMLImageElement;
      this.descriptionElement = this.querySelector('#pop-visitor-desc') as HTMLElement;
      
      if (!this.imageElement || !this.descriptionElement) {
        console.error('PopVisitorsFlashAnimation: Required elements not found');
      }
    }
  
    private loadConfiguration(): void {
      const displayTime = this.dataset.popDisplayTime;
      const flashDuration = this.dataset.popFlashDuration;
    ...
    private loadVisitorsData(): void {
      const visitorsDataAttribute = this.dataset.popVisitors;
      
      if (visitorsDataAttribute) {
        try {
          this.visitorsData = JSON.parse(visitorsDataAttribute);
        } catch (error) {
          console.error('PopVisitorsFlashAnimation: Failed to parse visitors data:', error);
          this.visitorsData = [];
        }
      }
    }
    ...
    private hideVisitor(callback: () => void): void {
      const imageContainer = this.querySelector('.pop-visitor-image') as HTMLElement;
      const description = this.querySelector('.pop-visitor-description') as HTMLElement;
      
      if (imageContainer && description) {
        imageContainer.classList.add('pop-visitor-hidden');
        description.classList.add('pop-visitor-hidden');
      }
    ...
    private flashVisitor(): void {
      ...
      imageContainer.classList.remove('pop-visitor-hidden');
      description.classList.remove('pop-visitor-hidden');
      
      imageContainer.classList.add('pop-visitor-flash', 'pop-visitor-visible');
      description.classList.add('pop-visitor-flash', 'pop-visitor-visible');
      
      // Remove flash class after animation completes
      setTimeout(() => {
        imageContainer.classList.remove('pop-visitor-flash');
        description.classList.remove('pop-visitor-flash');
    ...
    public updateVisitors(newVisitors: PopVisitor[]): void {
    ...
    public updateConfiguration(newConfig: Partial<PopVisitorsConfig>): void {
    ...
    public getVisitorsData(): PopVisitor[] {
  ...
  customElements.define('pop-visitors-animation', PopVisitorsFlashAnimation);
  
  // Export for potential external use
  export { PopVisitorsFlashAnimation, type PopVisitor, type PopVisitorsConfig };