...
class PopProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('pop-product-header')) {
    customElements.define('pop-product-header', PopProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('pop-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof PopProductHeader) {
      header.connectedCallback();
    }
  });
});

export { PopProductHeader };