// PopModalPurchaseInfo.ts - Simplified Implementation
...
class PopPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('pop-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('pop-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="pop-selection-item">
          <div class="pop-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="pop-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="pop-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="pop-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="pop-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="pop-direct-item">
        <div class="pop-direct-item-info">
          <div class="pop-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="pop-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="pop-direct-item-pricing">
          <div class="pop-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="pop-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="pop-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="pop-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'pop-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, PopPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof PopPurchaseModal) {
