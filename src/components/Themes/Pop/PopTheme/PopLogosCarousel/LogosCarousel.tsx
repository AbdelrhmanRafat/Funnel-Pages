      ...
      <div className="w-full max-w-6xl mx-auto px-4" data-component="logos-static">
        <div className="flex justify-center items-center gap-6 sm:gap-8 lg:gap-12 flex-wrap">
          {logos.map((logo, index) => (
            <div 
              key={index} 
              className="pop-logocarousel-div-card w-20 sm:w-24 lg:w-28 h-14 sm:h-16 lg:h-20 flex items-center justify-center p-3"
              style={{ 
      ...
      <div className="w-full max-w-6xl mx-auto px-4" data-component="logos-loading">
        <div className="flex justify-center items-center gap-6 sm:gap-8 lg:gap-12">
          {Array.from({ length: itemsPerView }).map((_, index) => (
            <div 
              key={index} 
              className="pop-logocarousel-div-skeleton w-20 sm:w-24 lg:w-28 h-14 sm:h-16 lg:h-20"
    ...
    >
      ...
      <div className="relative">
        ...
        {logos.length > itemsPerView && (
          <>
            <button 
              className="pop-logocarousel-button-left absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 w-10 h-10 sm:w-12 sm:h-12 lg:w-12 lg:h-12 rounded-full flex items-center justify-center transition-all duration-300 hover:shadow-lg z-10 border"
              onClick={goToPrev}
            ...
            <button 
              className="pop-logocarousel-button-right absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 w-10 h-10 sm:w-12 sm:h-12 lg:w-12 lg:h-12 rounded-full flex items-center justify-center transition-all duration-300 hover:shadow-lg z-10 border"
              onClick={goToNext}
        ...
        >
          ...
          >
            {logos.map((logo, index) => (
              ...
              >
                <div className="pop-logocarousel-div-card w-full h-16 sm:h-20 lg:h-24 flex items-center justify-center p-3 sm:p-4">
                  <img 
                    src={logo.image} 
            ...
            {maxIndex <= 5 ? (
              ...
              <nav className="flex justify-center gap-2" aria-label="Carousel pagination">
                {Array.from({ length: maxIndex + 1 }).map((_, index) => (
                  <button
                    key={index}
                    className={`pop-logocarousel-button-indicator w-2 h-2 rounded-full transition-all duration-300 ${
                      index === currentIndex ? 'pop-logocarousel-button-active w-6' : ''
                    }`}
              ...
              <div className="flex justify-center">
                <div className="w-32 sm:w-48 h-1 rounded-full overflow-hidden pop-logocarousel-div-progressbg">
                  <div 
                    className="h-full rounded-full transition-all duration-700 ease-out pop-logocarousel-div-progress"
