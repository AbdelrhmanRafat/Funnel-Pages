...
class MinimalProductHeader extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('minimal-product-header')) {
    customElements.define('minimal-product-header', MinimalProductHeader);
  }
});

document.addEventListener('astro:page-load', () => {
  const productHeaders = document.querySelectorAll('minimal-product-header:not(:defined)');
  productHeaders.forEach(header => {
    if (header instanceof MinimalProductHeader) {
      header.connectedCallback();
    }
  });
});

export { MinimalProductHeader };