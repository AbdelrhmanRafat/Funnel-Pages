// MinimalModalPurchaseInfo.ts - Simplified Implementation
...
class MinimalPurchaseModal extends HTMLElement {
  ...
  public open(): void {
    ...
    this.elements.modal.classList.add('minimal-modal-open');
  ...
  public close(): void {
    ...
    this.elements.modal.classList.remove('minimal-modal-open');
  ...
  private populateCustomOptions(customOptions: CustomOption[]): void {
    ...
    container.innerHTML = customOptions.map(option => `
      <div class="minimal-selection-item">
          <div class="minimal-panel-info">${getTranslation('modal.item', this.currentLang)} ${option.panelIndex}</div>
          <div class="minimal-selection-display"><span>${option.firstOption}</span></div>
${option.secondOption ? `<div class="minimal-selection-display"><span>${option.secondOption}</span></div>` : ""}
          </div>
    `).join('');
  }
  ...
  private createDirectItemHTML(option: any, quantity: number, price: number, discountPrice: number): string {
    ...
    if (option.firstOption) optionTags.push(`<span class="minimal-direct-option-tag">${option.firstOption}</span>`);
    if (option.secondOption) optionTags.push(`<span class="minimal-direct-option-tag">${option.secondOption}</span>`);

    return `
      <div class="minimal-direct-item">
        <div class="minimal-direct-item-info">
          <div class="minimal-direct-item-title">المنتج المحدد</div>
          ${optionTags.length > 0 ? `<div class="minimal-direct-item-options">${optionTags.join('')}</div>` : ''}
        </div>
        <div class="minimal-direct-item-pricing">
          <div class="minimal-direct-item-price">${discountPrice} $</div>
          ${hasDiscount ? `<div class="minimal-direct-item-original-price">${price} $</div>` : ''}
          ${hasDiscount ? `<div class="minimal-direct-item-discount">وفر ${price - discountPrice} $</div>` : ''}
        </div>
        <div class="minimal-direct-item-quantity">${quantity}</div>
      </div>
    `;
  }
...
class ModalManager {
  private static readonly COMPONENT_NAME = 'minimal-purchase-modal';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, MinimalPurchaseModal);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedModals.forEach(modal => {
      if (modal instanceof MinimalPurchaseModal) {
