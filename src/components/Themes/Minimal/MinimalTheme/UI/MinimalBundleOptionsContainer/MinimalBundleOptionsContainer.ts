// MinimalSelectOptionsBundles.ts - Enhanced with Fixed Naming Convention
...
const CSS_CLASSES = {
  // Selection states
  SELECTED_COLOR: 'minimal-bundle-options-container-selected-color',
  SELECTED_SIZE: 'minimal-bundle-options-container-selected-size',
  SELECTED_GENERIC: 'minimal-selected',
  
  // Option types
  COLOR_OPTION: 'minimal-bundle-options-container-color-option',
  SIZE_OPTION: 'minimal-bundle-options-container-size-option',
  
  // Availability states
  OPTION_AVAILABLE: 'minimal-bundle-options-container-option-available',
  OPTION_DISABLED: 'minimal-bundle-options-container-option-disabled',
  OPTION_TRANSITIONING: 'minimal-bundle-options-container-option-transitioning'
} as const;

class MinimalSelectOptionsBundles extends HTMLElement implements Observer<BundleState>, Observer<CustomOptionBundlesState> {
  ...
  private initializeSettings(): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to initialize settings:', error);
    }
  }
  ...
  private initializeElements(): void {
    ...
    if (!this.firstOptionElements?.length && !this.secondOptionElements?.length) {
      console.warn('MinimalSelectOptionsBundles: No option elements found');
    }
  }
  ...
  public update(subject: Subject<BundleState | CustomOptionBundlesState>): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Observer update failed:', error);
    }
  }
  ...
  private handleFirstOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('MinimalSelectOptionsBundles: First option selection failed:', error);
    }
  }
  ...
  private handleSecondOptionClick(element: HTMLElement): void {
    ...
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Second option selection failed:', error);
    }
  }
  ...
  private applySelectionStyle(element: HTMLElement): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to apply selection style:', error);
    }
  }
  ...
  private filterSecondOptions(firstValue: string): void {
    ...
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to filter second options:', error);
    }
  }
  ...
  private toggleElementAvailability(element: HTMLElement, isAvailable: boolean): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to toggle element availability:', error);
    }
  }
  ...
  private syncUIWithObserver(panelOption: any): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to sync UI with observer:', error);
    }
  }
  ...
  private updateVisualSelections(panelOption: any): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to update visual selections:', error);
    }
  }
  ...
  private findSkuId(): number | null {
    ...
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to find SKU ID:', error);
      return null;
    }
  }
  ...
  private findImageUrl(): string | null {
    ...
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to find image URL:', error);
      return null;
    }
  }
  ...
  public clearSelections(): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to clear selections:', error);
    }
  }
  ...
  public forceRefreshSelections(): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to refresh selections:', error);
    }
  }
  ...
  public validateSelectionStates(): boolean {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to validate selection states:', error);
      return false;
    }
  }
  ...
  public repairSelections(): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to repair selections:', error);
    }
  }
  ...
  public resetToInitialState(): void {
    try {
      ...
      console.error('MinimalSelectOptionsBundles: Failed to reset to initial state:', error);
    }
  }
...
class MinimalSelectOptionsBundlesManager {
  private static readonly COMPONENT_NAME = 'minimal-select-options-bundles';
  ...
  private static registerComponent(): void {
    if (!customElements.get(this.COMPONENT_NAME)) {
      customElements.define(this.COMPONENT_NAME, MinimalSelectOptionsBundles);
    }
  }
  ...
  private static handleAstroPageLoad(): void {
    ...
    undefinedComponents.forEach(component => {
      if (component instanceof MinimalSelectOptionsBundles) {
...
MinimalSelectOptionsBundlesManager.initialize();

export { MinimalSelectOptionsBundles };