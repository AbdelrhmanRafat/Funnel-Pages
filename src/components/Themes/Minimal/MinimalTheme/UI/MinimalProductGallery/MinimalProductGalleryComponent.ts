// MinimalGalleryComponent.ts - Web Component for Gallery
...
class MinimalGallery extends HTMLElement {
    ...
    private updateThumbnails(): void {
        this.elements.thumbnails?.forEach((thumbnail, index) => {
            thumbnail.classList.remove('minimal-thumbnail-active');
            if (index === this.currentIndex) {
                thumbnail.classList.add('minimal-thumbnail-active');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!customElements.get('minimal-gallery')) {
        customElements.define('minimal-gallery', MinimalGallery);
    }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
    const galleries = document.querySelectorAll('minimal-gallery:not(:defined)');
    galleries.forEach(gallery => {
        if (gallery instanceof MinimalGallery) {
            gallery.connectedCallback();
        }
    });
});

export { MinimalGallery };