// MinimalThemeProductFunnel.ts - Web Component for Product Funnel
...
class MinimalProductFunnel extends HTMLElement implements Observer<any> {
...
document.addEventListener('DOMContentLoaded', () => {
  if (!customElements.get('minimal-product-funnel')) {
    customElements.define('minimal-product-funnel', MinimalProductFunnel);
  }
});

// Handle Astro page transitions
document.addEventListener('astro:page-load', () => {
  const productFunnels = document.querySelectorAll('minimal-product-funnel:not(:defined)');
  productFunnels.forEach(funnel => {
    if (funnel instanceof MinimalProductFunnel) {
      funnel.connectedCallback();
    }
  });
});

export { MinimalProductFunnel };