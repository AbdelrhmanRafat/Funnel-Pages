...
import './MinimalThemeRates.css';

interface MinimalThemeRatesReactProps {
...
const MinimalThemeRatesReact: React.FC<MinimalThemeRatesReactProps> = ({
  ratingData,
  currentLang,
}) => {
  ...
  const StarIcon: React.FC<{ star: Star }> = ({ star }) => {
    ...
    switch (star.type) {
      case 'full':
        return (
          <svg className="minimal-rating-star-full" {...baseProps}>
      ...
      case 'half':
        return (
          <svg className="minimal-rating-star-half" {...baseProps}>
            <defs>
              <linearGradient id="halfGradient">
                <stop offset="50%" stopColor="currentColor" />
                <stop offset="50%" stopColor="var(--minimal-color-outline)" />
              </linearGradient>
            </defs>
      ...
      case 'empty':
        return (
          <svg className="minimal-rating-star-empty" {...baseProps}>
            <path d={starPath} fill="currentColor" />
          </svg>
        );
  ...
  return (
    <div id={FunnelMinimalComponents.MinimalRates} className="minimal-rates">
      <div
        className="minimal-rating-display"
      ...
      >
        ...
        <span className="minimal-rating-number">({rating})</span>
      </div>
    </div>
  );
};

export default MinimalThemeRatesReact;